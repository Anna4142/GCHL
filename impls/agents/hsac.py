...
class HSACAgent(flax.struct.PyTreeNode):
    ...
    def extract_subgoal_weights(self, rewards: jnp.ndarray, masks: jnp.ndarray) -> jnp.ndarray:
        ...
        Args:
            rewards: Array of rewards for each timestep in the batch.
            masks: Array indicating non-terminal (1) or terminal (0) transitions.

        Returns:
            subgoal_weights: Array of weights corresponding to each sample in the batch.
        """
        # Initialize weights to 1
        subgoal_weights = jnp.ones_like(rewards)

        # Identify indices where rewards are received
        reward_indices = jnp.where(rewards > 0)[0]

        # Assign higher weights to samples preceding reward events
        # For simplicity, assign a weight of 2 to the state-action pair preceding a reward
        # and leave others as 1
        # This can be adjusted based on specific requirements

        def assign_weights(idx, weights):
            # Avoid out-of-bounds
            condition = idx > 0
            return jax.lax.cond(
                condition,
                lambda: weights.at[idx - 1].multiply(2.0),
                lambda: weights,
            )

        subgoal_weights = jax.vmap(assign_weights)(reward_indices, jnp.zeros_like(reward_indices))
        # Flatten and ensure the weights align with the batch size
        # This simplistic approach may need refinement for overlapping subgoals

        # Alternative approach: For all samples, if the next state has a reward, weight current sample higher
        # This avoids handling multiple rewards and overlapping weights
        next_rewards = jnp.roll(rewards, -1)
        next_masks = jnp.roll(masks, -1)
        weight_multiplier = 2.0
        subgoal_weights = jnp.where((next_rewards > 0) & (next_masks > 0), weight_multiplier, 1.0)
